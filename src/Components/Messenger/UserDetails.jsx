...
function UserDetails({}) {
  ...
  return senderData ? (
    <div className="chatRoomUserDetailsContainer flexBox scroll">
      <div
        className="chatRoomUserDetailsImage"
        onClick={() => navigate(`/profile/${senderData?.uid}`)}
      >
        {activeState && <StatusDot width="18px" height="18px" right="15px" />}
        <img
          src={senderData?.profilePic || "/Images/userProfile_icon.png"}
          alt="User"
        />
      </div>
      <div
        className="chatRoomUserDetailsData"
        onClick={() => navigate(`/profile/${senderData?.uid}`)}
      >
