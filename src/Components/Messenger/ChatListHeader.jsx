...
function ChatListHeader() {
  ...
  const result = users.filter((user) => {
    return (
      ...
      JSON.stringify(friends).includes(user.uid) &&
      query
    );
  });
  return (
    <div className="chatListHeaderContainer">
      ...
      <div
        className="messengerResults scroll"
        style={{ display: query ? "block" : "none" }}
      >
        {result.length
          ? result.map((user) => (
              <UserResultCard key={user.uid} {...user} setQuery={setQuery} />
            ))
          : query && <div className="noFriendsFound">No friends found</div>}
      </div>
    </div>
  );
}
