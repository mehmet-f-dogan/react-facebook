...
function NewPostModal({ togglePostModal }) {
  ...
  const { first_name, profilePic, uid, last_name } = useSelector(
    ...
    if (imageRef.current?.files[0]?.name) {
      if (imageRef.current?.files[0]?.type.includes("image")) {
        const uploadTask = storage
          .ref(`postImages/${imageRef.current.files[0].name}`)
              ...
              .then((url) => {
                Compress.imageFileResizer(
                  ...
                  (uri) => {
                    let file = new File(
                      [uri],
                      "thumb_" + imageRef.current.files[0].name,
        ...
        const uploadTask = storage
          .ref(`postVideos/${imageRef.current.files[0].name}`)
          .put(imageRef.current.files[0]);
  ...
  return (
    <div className="createNewPostModal" onClick={togglePostModal}>
      <PopUp
        className="createNewPostContainer"
        onClick={() => {
          closeEmojiMart();
          closeActivityBox();
        }}
      >
        ...
        <div className="createNewPostHeader flexBox">
          ...
          <div
            className="createNewPostCloseButton flexBox"
        ...
        <div className="createNewPostInput flexBox">
          ...
          {imageUrl && (
            <div className="blobImage">
              ...
              <div
                className="blobImageCloseButton flexBox"
                onClick={handleBlobClose}
              >
                <CloseIcon />
              </div>
            </div>
        ...
        <div className="newPostAdd flexBox">
          ...
          <PopUp className="flexBox iconWrap">
            ...
            {activityBox && (
              <div className="activityBox scroll flexBox">
                {activities.map((el) => (
                  <div
                    key={el.activity}
                    className="flexBox activityList"
                    onClick={() => handleActivity(el)}
                  >
