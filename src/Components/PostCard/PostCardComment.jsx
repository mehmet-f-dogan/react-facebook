...
function PostCardComment({
  ...
  return (
    <PopUp className="postCardCommentContainer flexBox">
      {comments.length > 2
        ? limit === 2 && (
            <p onClick={() => setLimit(comments.length)}>
              View more {comments.length - 2} comments
            </p>
          )
        : null}
      {comments
        ?.filter((el, i) => comments.length - limit <= i)
        .map((el) => (
          <CommentBox key={el.commentId} {...el} />
        ))}
      {limit === comments.length && comments.length > 2 && (
        <p onClick={() => setLimit(2)}>View less comments</p>
      )}
