...
function ActiveContactSideBar({ toggleNewChatBox }) {
  ...
  const { activeContacts, users } = useSelector(
    (state) => state.app,
    shallowEqual
  );
  const { friends, user, userActiveStatus } = useSelector(
    (state) => state.auth,
    shallowEqual
  );
        ...
        <div className="sideBarLinksContainer activeContact">
          <div className="flexBox activeContactHeader">
            ...
            {activeStatusBox && (
              <React.Fragment>
                {userActiveStatus ? (
                  <PopUp
                    className="activeStatusPopUpCover flexBox"
                    onClick={() => dispatch(updateUserActiveStatus())}
                  >
          ...
          <React.Fragment>
            {activeContacts.map((user) => (
              <ActiveContacts key={user.friendId} {...user} />
            ))}
          </React.Fragment>
        </div>
      )}
