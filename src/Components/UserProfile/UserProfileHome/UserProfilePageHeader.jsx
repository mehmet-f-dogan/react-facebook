...
function UserProfilePageHeader({
  coverPhoto,
  currentUser,
  forceRefresh,
  userProfileDetails,
    ...
    if (coverPicImageRef.current?.files[0]?.name) {
      if (coverPicImageRef.current?.files[0]?.type.includes(`image`)) {
        const uploadTask = storage
          .ref(`coverPicImages/${coverPicImageRef.current.files[0].name}`)
          .put(coverPicImageRef.current.files[0]);
              ...
              .then((url) => {
                Compress.imageFileResizer(
                  ...
                  (uri) => {
                    let file = new File(
                      [uri],
                      "thumb_" + coverPicImageRef.current.files[0].name,
                        ...
                        storage
                          ...
                          .then((thumb_url) => {
                            ...
                            database.collection("users").doc(uid).update({
                              coverPic: url,
                              coverPicPath:
                                coverPicImageRef.current.files[0].name,
                            });
                            database
                              .collection("posts")
                              .add(payload)
  ...
  return (
    <div className="userProfileContainer">
      <div
        className="userProfileCoverPageBox"
        style={{ backgroundImage: `url("${coverPhoto}")` }}
      >
        <div className="userProfileCoverPageOverlay">
          <div
            className="userProfileCoverPhotoBox"
            style={
              true
          ...
          >
            ...
            {currentUser === uid && (
              <React.Fragment>
                <div
                  className="userProfileEditCoverPhotoContainer flexBox"
                  onClick={toggleShowCoverPicModal}
                  title="Edit cover photo"
                >
                  <div className="userProfileEditCoverPhotoBox">
                    <img
                      className="userProfileEditCoverCamIcon"
                      src={
                      ...
                      alt="camera"
                    />
                    <span className="userProfileEditCoverNamePlate">
                      Edit Cover Photo
                    </span>
                  </div>
                </div>
                        ...
                        >
                          ...
                          <div className="profilePicPreviewNoteBox flexBox">
                            <div className="chooseProfilePicInputBox">
                              <label htmlFor="coverPicFileInput">
                                Cover Pic
                              </label>
                              <input
                                ref={coverPicImageRef}
                                id="coverPicFileInput"
                                type="file"
                                onChange={handleChangeCoverPicPreview}
                        ...
                        {coverPicImagePreview && (
                          <React.Fragment>
                            <div className="profilePicPreviewImage">
                              <img
                                src={coverPicImagePreview}
                                alt="coverImage"
                              />
                            </div>
                            <div
                              className="profilePicPreviewCloseIconBox flexBox"
                              onClick={handleRemoveCoverPicPreview}
                            >
                              <CloseIcon />
                            </div>
                          </React.Fragment>
      ...
      <div className="userNameContainer">
        ...
        <React.Fragment>
          {userProfileDetails.userBio && (
            <p className="userProfileBioContent">
              {userProfileDetails.userBio}
            </p>
          )}
          {currentUser === uid &&
            !addBioVisibility &&
            !userProfileDetails.userBio && (
              <p className="addUserBio" onClick={handleAddBioVisibility}>
                Add Bio
              </p>
            )}
          {currentUser === uid &&
          ...
          {currentUser === uid && addBioVisibility && (
            <PopUp
              className="addBoxTexFieldContainer"
              onClick={(e) => {
                toggleShowEmojiMart(e);
                closeEmojiMart();
              }}
            >
              <textarea
                className="addBioTextField scroll"
              ...
              <div className="flexBox addBioButtonContainer">
                <div className="flexBox">
                  <img
                    className="addBioGlobeIcon"
                    src={
                      import.meta.env.VITE_PUBLIC_URL +
                      "/Images/public_globe_icon.png"
                    }
                    alt="Public"
                  />
                  <span>Public</span>
                ...
                <div className="addBioButtonBox">
                  ...
                  <button
                    disabled={
                      textFieldQuery === "" || textFieldQuery.length > 101
                    }
