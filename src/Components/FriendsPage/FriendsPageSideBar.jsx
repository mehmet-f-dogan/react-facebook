...
function FriendsPageSideBar({ forceRefresh }) {
  ...
  const [friendsVisibilityLimitation, setFriendsVisibilityLimitation] =
    useState(false);
  ...
  return (
    <div className="friendsPageSideBarContainer">
      {friendRequests.length > 0 ? (
        !friendsVisibilityLimitation ? (
          <React.Fragment>
            <h1 className="friendsSideBarHeading">Friends</h1>
            <div className="flexBox friendsSideBarCount">
              <h4>{`${friendRequests.length} ${
                friendRequests.length > 1 ? `Friend Requests` : `Friend Request`
              }`}</h4>
              <div
                className="seeAllFriends"
                onClick={() => setFriendsVisibilityLimitation(true)}
              >
                See All
              </div>
            </div>
            <div>
              {friendRequests.map((el) => {
                return i < 4 && <FriendsPageCard key={el.senderId} {...el} />;
              })}
            </div>
          </React.Fragment>
        ) : (
          <React.Fragment>
            <p className="seeAllFriendsBreadCrumbs flexBox">
              <small onClick={() => setFriendsVisibilityLimitation(false)}>
                Friends
              </small>
              <span>{`>`}</span>
            ...
            <p className="allFriendsRequestsSideBarCount">{`${
            ...
            <div className="seeAllSentRequestsTagBox">
              <p
                className="seeAllSentRequestsTag"
                onClick={toggleSentRequestVisibility}
              >
                View Sent Requests
              </p>
            </div>
            ...
            {peopleSuggested?.map((el, i) => {
              return (
                ...
                i < 5 && (
                  <div
                    key={el.uid}
                    onClick={() => navigate(`/friends/${el.uid}`)}
                    className="flexBox sideBarContentLink"
                  >
                    <SideBarContent
      ...
      {friends.length !== 0 && (
        <React.Fragment>
          ...
          <div className="sideBarLinksContainer">
            ...
            {friends?.map((el, i) => {
              return <FriendsCard key={el.friendId} {...el} />;
            })}
          </div>
        </React.Fragment>
      )}
      {sentRequestVisibility && (
        <SentFriendRequestModal
          sentRequests={sentRequests}
          toggleSentRequestVisibility={toggleSentRequestVisibility}
        />
      )}
    </div>
  );
