...
function SenderFriendCard({ receiverId }) {
  const [sentFriendRequestDetails, setSentFriendRequestDetails] =
    useState(null);
  ...
  const handleCancelRequest = () => {
    database
      .collection("users")
      .doc(sentFriendRequestDetails.uid)
      .collection("friendRequests")
      .doc(`${sentFriendRequestDetails.uid}${uid}`)
      .delete();
    database
      .collection("users")
      .doc(uid)
  ...
  return sentFriendRequestDetails ? (
    <div className="flexBox senderFriendCardMainContainer">
      <div className="flexBox">
        <Link
          className="senderProfilePicCover"
          to={`/profile/${sentFriendRequestDetails.uid}`}
        >
          <img
            className="senderProfilePicImage"
            src={
              sentFriendRequestDetails.profilePic ||
              "/Images/userProfile_icon.png"
            }
            alt={sentFriendRequestDetails.first_name}
          />
        </Link>
      </div>
      <div className="senderProfileNamePlateCover flexBox">
        <Link
          className="senderProfileNamePlate"
          to={`/profile/${sentFriendRequestDetails.uid}`}
        >
      ...
      >
        <img
          ...
          src={"/Images/cancel_request_icon.png"}
          alt="plus"
        />
