...
function Chats({ newChatBox, toggleNewChatBox }) {
  ...
  const { activeMessages, inActiveMessages } = useSelector(
    (store) => store.app
  );
  ...
  return (
    <>
      <PopUp
        className="activeChatBubbleContainer"
        onMouseEnter={openOption}
        onMouseLeave={() => !optionBox && closeOption()}
      >
        {option && (activeMessages.length || inActiveMessages.length) ? (
          <PopUp className="bubbleOptions flexBox" onClick={handleOption}>
            <DotsIcon />
            {optionBox && (
              <PopUp className="bubbleOptionBox">
                <div
                  className="flexBox bubbleOptionIconBox"
        ...
        <div className="activeChatBubbleBox">
          {inActiveMessages.map((chatRoom) => (
            <ActiveChatBubble key={chatRoom.chatID} {...chatRoom} />
          ))}
        </div>
        <ChatBubble toggleNewChatBox={toggleNewChatBox} />
        <div className="chatsContainer flexBox">
          {activeMessages.map((chatRoom, i) => (
            <ChatBox
              key={chatRoom.chatID}
              {...chatRoom}
              active={activeChatIndex === i}
              handleActiveChatBox={handleActiveChatBox}
              index={i}
            />
          ))}
