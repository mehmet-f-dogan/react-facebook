import {
  ADD_ACTIVE_MESSAGE,
  ADD_IN_ACTIVE_MESSAGE,
  ADD_IN_ACTIVE_MESSAGE_TO_ACTIVE_MESSAGE,
  CLOSE_ALL_MESSAGE,
  GET_CHAT_ROOMS,
  GET_USERS,
  MINIMIZE_ALL_MESSAGE,
  REMOVE_ACTIVE_MESSAGE,
  REMOVE_IN_ACTIVE_MESSAGE,
  RESET_APP,
...
export const appReducer = (state = init, { type, payload }) => {
  switch (type) {
    case ADD_ACTIVE_MESSAGE: {
      ...
      const updatedInActiveMessages = state.inActiveMessages.filter(
    ...
    case ADD_IN_ACTIVE_MESSAGE: {
      ...
      const updateActiveMessage = state.activeMessages.filter(
    ...
    case REMOVE_IN_ACTIVE_MESSAGE: {
      return {
        ...state,
        inActiveMessages: state.inActiveMessages.filter(
          (item) => item.chatID !== payload
        ),
      };
    }
    ...
    case ADD_IN_ACTIVE_MESSAGE_TO_ACTIVE_MESSAGE: {
      ...
      const updateInActiveMessage = state.inActiveMessages.filter(
        (el) => el.chatID !== payload
      );
