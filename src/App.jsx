...
function App() {
  ...
  React.useEffect(() => {
    if (uid && isAuth) {
      ...
      const unsubscribe4 = database
        .collection("users")
        .doc(uid)
        .collection("friends")
        .onSnapshot((res) => {
          const newFriends = res.docs.map((doc) => ({
            docId: doc.id,
            ...doc.data(),
          }));
      ...
      const unsubscribe6 = database
        .collection("chatRooms")
        .where("authors", "array-contains", uid)
        .onSnapshot((res) => {
          const newChatRooms = res.docs.map((doc) => ({
            chatID: doc.id,
            ...doc.data(),
          }));
          dispatch(getChatRooms(newChatRooms));
        });
      const unsubscribe7 = database
        .collection("users")
        .doc(uid)
        .onSnapshot((doc) => {
          dispatch(loginSuccess(doc.data()));
        });
      const unsubscribe8 = database
        .collection("users")
        .doc(uid)
        .collection("notifications")
        .orderBy("time", "desc")
        .onSnapshot((res) => {
          const newNotifications = res.docs.map((doc) => ({
            notificationID: doc.id,
            ...doc.data(),
          }));
          dispatch(getNotifications(newNotifications));
        });
      const unsubscribe9 = database
        .collection("users")
        .doc(uid)
